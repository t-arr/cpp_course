...
void displayPNGInfo(const LodePNGInfo& info)
{
    ...
    {
        std::cout << "Text: " << info.itext_keys[i] << ", " << info.itext_langtags[i] << ", " << info.itext_transkeys[i] << ": " << info.itext_strings[i]
                  << std::endl
                  << std::endl;
    }
...
void displayAsciiArt(const std::vector<unsigned char>& image, unsigned w, unsigned h)
{
    if (w > 0 && h > 0)
    {
        ...
        if (h2 > (w2 * 2))
            h2 = w2 * 2; // avoid too large output
        ...
        for (unsigned y = 0; y < h2; y++)
        {
            ...
            for (unsigned x = 0; x < w2; x++)
            {
                ...
                if (saturation > 32)
                {
                    ...
                    if (h)
                        letter = (min == r ? 'c' : (min == g ? 'm' : 'y'));
                    else
                        letter = (max == r ? 'r' : (max == g ? 'g' : 'b'));
                }
                int symbol = ' ';
                if (lightness > 224)
                    symbol = '@';
...
int main(int argc, char* argv[]) /*list the chunks*/
{
    ...
    {
        if (std::string(argv[i]) == "--ignore_checksums")
    ...
    if (w > 0 && h > 0)
    {
        std::cout << "Top left pixel color:" << " r: " << (int)image[0] << " g: " << (int)image[1] << " b: " << (int)image[2] << " a: " << (int)image[3]
                  << std::endl;
    }
